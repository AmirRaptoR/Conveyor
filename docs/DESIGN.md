...
| 5 | Moving a column runs a script, logs visible | the script the source declares for that stage's name. stdout/stderr stream line-by-line to the UI over SSE; exit code decides the next stage. |

Mocked from day one: `providers/mock/` and `agents/mock/`
exercise every path —
...
is just a transition, so it calls `move` and then the stage script immediately, through
...
arbitrary executables, so a Python script that already works keeps working as a
stage. Port one only when there is a reason.
...
real provider does, and the board is easier to design once real items exist.
