...
if [[ -z "$(jq -r '.item.description // ""' <<<"$payload")" ]]; then
  ...
  jq -n '{blocked:"needs a description before it can be refined"}' >"$CONVEYOR_RESULT"
  exit 20
fi
