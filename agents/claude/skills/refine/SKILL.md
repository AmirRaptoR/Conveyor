...
- **Unattended:** decide the batches yourself, same self-answer-and-record discipline as §2, then act immediately, before any spec gets drafted:
  ...
  3. **Link every child to the parent with GitHub's native sub-issue relationship**, not just the text mention — it's what drives the parent's progress bar:
     ...
     ```
  4. **If a PR already exists against the parent** (an implementer got ahead of a refine that should have split earlier): make the child whose scope matches that PR's actual diff, then create `issue-<child>` at the old PR's head commit and open a replacement PR whose body contains both `Closes #<child>` and `<!-- conveyor:item <child> -->`. Copy the old PR's title, base and relevant discussion, close the old PR only after the replacement exists, and give the child the `conveyor:*` label matching wherever the PR already sits in the pipeline (e.g. `conveyor:review`). Do not rename an open PR's head branch: GitHub closes the PR instead of retargeting it. Never change only one identity signal: Conveyor rejects a marker/body/branch disagreement rather than guessing which item owns the PR.
