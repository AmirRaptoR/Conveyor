...
func runPreflight(ctx context.Context, cfg *config.Config, r *runner.Runner, only string, out io.Writer) bool {
	...
	for _, s := range cfg.Sources {
		...
		// could not even start the script (no resolved workdir, no
		// resolved provider) — the engine-side failures above already say
		// what is wrong, and running the script anyway would only say it
		// worse.
		...
		// Agents: one check per distinct agent this source's scripts name,
		// in the order the stages that name them appear in the config —
		// the only deterministic order available, since scripts: is a map.
